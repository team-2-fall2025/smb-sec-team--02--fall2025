...
from bson import ObjectId  # noqa: F401  # (kept in case it's used elsewhere)
...
#     assets = db["assets"]
#     intel = db["intel_events"]
#     links = db["asset_intel_links"]
...
#         if not a.get("type"):
#             patch["type"] = _infer_type(a.get("name"))

#         if not a.get("criticality"):
#             patch["criticality"] = _crit_from_sens(a.get("data_sensitivity"))
...
#                 {
#                     "at": now,
#                     "actor": "identify_agent",
#                     "entity": "asset",
#                     "entity_id": a["_id"],
#                     "changes": [
#                         {"field": k, "old": a.get(k), "new": v} for k, v in patch.items()
#                     ],
#                 }
#             )
#             print(patch)
...
#         print(ie)
#         async for a in assets.find(
#             {"ip": ie.get("indicator"), "deleted_at": {"$exists": False}}
#         ):
...
#     async for ie in intel.find(
#         {"created_at": {"$gte": since}, "indicator_type": {"$in": ["hostname", "domain"]}}
#     ):
#         dom = _norm(ie.get("indicator"))
#         async for a in assets.find(
#             {"hostname": {"$exists": True}, "deleted_at": {"$exists": False}}
#         ):
#             h = _norm(a.get("hostname"))
#             if not h:
#                 continue

#             if ie["indicator_type"] == "hostname":
#                 is_match = h == dom
#             else:
#                 # indicator_type == "domain"
#                 is_match = h == dom or h.endswith("." + dom)
