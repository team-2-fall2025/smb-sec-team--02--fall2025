#!/usr/bin/env python3
"""
MongoDB setup script (NO validators):
- Creates collections if missing (no JSON Schema)
- Ensures indexes from scripts/schemas/indexes.json and any *.indexes.json
- Seeds documents from seed/*.json (Extended JSON supported)
...
def ensure_indexes(coll_name: str, specs: List[Dict[str, Any]]) -> None:
    if not specs:
        return
    for item in specs:
        keys = item.get("keys", {})
...
def seed_controls(payload: List[Dict[str, Any]]) -> None:
    now = datetime.utcnow()
    upserted = 0
    for c in payload:
...
def seed_control_mappings(payload: List[Dict[str, Any]]) -> None:
    now = datetime.utcnow()
    upserted = 0
    for m in payload:
...
def seed_policies(payload: List[Dict[str, Any]]) -> None:
    now = datetime.utcnow()
    upserted = 0
    for p in payload:
...
def seed_policy_assignments(payload: List[Dict[str, Any]]) -> None:
    now = datetime.utcnow()
    upserted = 0
    for pa in payload:
...
def seed_control_evidence(payload: List[Dict[str, Any]]) -> None:
    now = datetime.utcnow()
    ...
    for ev in payload:
        ...
        try:
            ...
            pass
    print(f"[OK] Seed control_evidence: upserted_or_matched≈{inserted_or_upserted}, total_payload={len(payload)}")
...
def main() -> None:
    ...
    if evidence_seed:
        seed_control_evidence(evidence_seed)
    else:
